...
export const getMatches = async (url = "", filterBy = "", setHeaders = {}) => {
  const setUrl = url === "" ? `${corsAnywhere + yourUrl}` : url;
  // const setUrl =
  //   url === "" ? `http://localhost:3001/matches-information?_delay=2000` : url;
  ...
  if (!getLocalStorage("matches")) {
    localStorage.setItem("matches", JSON.stringify(resp.data));
  }
